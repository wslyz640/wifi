...
class WIFIRBAC {
    ...
    static public function AccessDecision($appName=GROUP_NAME) {
        ...
        if(WIFIRBAC::checkAccess()) {
            ...
            if(empty($_SESSION[C('ADMIN_AUTH_KEY')])) {
                ...
                if(!in_array(strtoupper($appName).'/'.strtoupper($module).'/'.strtoupper(ACTION_NAME),$accessList)) {
    ...
    static public function getAccessList($authId) {
        ...
        $sql    =   "select node.id,node.g,node.m,node.a from ".
                    $table['role']." as role,".
                    $table['user']." as user,".
                    $table['access']." as access ,".
                    $table['node']." as node ".
                    "where user.user_id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.status=1";
        $apps =   $db->query($sql);
        //log::write($sql);
        $access =  array();
...
/**
     ...
     * @access public
     +----------------------------------------------------------
     */
    static public function getAccessIDList($authId) {
        // Db方式权限数据
        $db     =   Db::getInstance(C('RBAC_DB_DSN'));
        $table = array('role'=>C('RBAC_ROLE_TABLE'),'user'=>C('RBAC_USER_TABLE'),'access'=>C('RBAC_ACCESS_TABLE'),'node'=>C('RBAC_NODE_TABLE'));
        $sql    =   "select node.id,node.g,node.m,node.a from ".
